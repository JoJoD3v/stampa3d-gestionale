...
@section('title', 'Modifica Lavoro ' . $lavoro->numero)
@section('page-title', 'Modifica Lavoro')

@section('content')

<div class="page-header">
    <h1>Modifica {{ $lavoro->numero }}</h1>
    <div style="display:flex;gap:.5rem;">
        <a href="{{ route('backend.lavori.show', $lavoro) }}" class="btn btn-secondary">
...
<form method="POST" action="{{ route('backend.lavori.update', $lavoro) }}" id="lavoroForm">
            ...
            <div class="card-body">
                    ...
                    <div style="display:flex;gap:.5rem;align-items:center;">
                        <select id="customer_id" name="customer_id"
                            ...
                            @foreach($customers as $c)
                                <option value="{{ $c->id }}"
                                    {{ old('customer_id', $lavoro->customer_id) == $c->id ? 'selected' : '' }}>
                                    {{ $c->full_name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="preventivo" name="preventivo" step="0.01" min="0"
                           ...
                           value="{{ old('preventivo', $lavoro->preventivo) }}" placeholder="0.00">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="scadenza" name="scadenza"
                           ...
                           value="{{ old('scadenza', $lavoro->scadenza?->format('Y-m-d')) }}">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status"
                        ...
                        @foreach(\App\Models\Lavoro::STATUS_LABELS as $val => $label)
                            <option value="{{ $val }}"
                                {{ old('status', $lavoro->status) === $val ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="note" name="note" rows="4"
                              ...
                              placeholder="Note aggiuntive…">{{ old('note', $lavoro->note) }}</textarea>
    ...
    <div style="margin-top:1.5rem;display:flex;justify-content:flex-end;gap:.75rem;">
        <a href="{{ route('backend.lavori.show', $lavoro) }}" class="btn btn-secondary">Annulla</a>
